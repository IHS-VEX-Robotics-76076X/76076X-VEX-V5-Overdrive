...
void Chassis::set_tracking_wheel(pros::Rotation *wheel, double wheelDiameterInch) {
    ...
    // A pros::Rotation counts in centidegrees: 36000 per full turn of the
    // wheel. One full turn moves the robot one circumference, which is
    // (diameter * pi) inches. So each centidegree is worth:
    //
    //     (diameter * pi) / 36000  inches
    //
    // Working it out once here keeps it out of the loop below.
    trackingWheelInchesPerCentidegree = (wheelDiameterInch * M_PI) / 36000.0;
}

void Chassis::odomLoop() {
    // Prefer the tracking wheel when one is installed. It's unpowered, so it
    // can't spin uselessly the way a driven wheel does when it slips.
    bool useTrackingWheel = (trackingWheel != nullptr);

    // A tracking wheel can be *configured* without actually being *plugged
    // in* - which is exactly what happens when you're bench-testing with only
    // the drivetrain wired up. Every read then fails, and without the counter
    // below odometry would sit frozen at its starting position for the entire
    // run, silently, with nothing to indicate why.
    //
    // So: if the wheel never answers even once, give up on it and use the
    // drive encoders instead. Only bail out while still unseeded - if the
    // wheel HAS worked before and then dropped out mid-run, that's a
    // disconnection, and the right response is to wait for it rather than
    // switch sources. (Switching after seeding would also be a bug: the
    // baseline is in centidegrees but the encoders report ticks, so the first
    // delta across the switch would be garbage.)
    int trackingWheelFailures = 0;
    constexpr int TRACKING_WHEEL_GIVE_UP_TICKS = 50; // ~0.5s at 10ms per tick

    // To work out how far we moved, we compare this tick's sensor reading
    // against last tick's. That needs a starting reading to compare against -
    // a "baseline".
    //
    ...
    // error value. Storing that as the baseline would corrupt every distance
    // calculated afterward, because every later reading gets compared against
    // garbage. Waiting for a good reading avoids that.
    bool seeded = false;
    ...
    while (odomRunning) {
        ...
        //
        // Note there's no sideways ("strafe") term. With a single
        // forward-facing wheel there is simply no way to measure lateral
        // drift, so if the robot gets shoved sideways, odometry won't see it.
        double deltaForward = 0.0;

        if (useTrackingWheel) {
            // An unplugged or broken Rotation sensor doesn't return a small or
            // zero value - it returns PROS_ERR, a huge error code. Adding that
            // to our position would fling the robot thousands of inches across
            // the field in one tick, and nothing can undo it afterward. So we
            // throw the reading away instead. We lose whatever movement
            // happened during the disconnection, which is far better than
            // corrupting the position permanently.
            std::int32_t raw = trackingWheel->get_position();

            if (raw != PROS_ERR) {
                trackingWheelFailures = 0;
                double reading = raw;
                if (seeded) {
                ...
                prevReading = reading;
            } else if (!seeded && ++trackingWheelFailures >= TRACKING_WHEEL_GIVE_UP_TICKS) {
                // Never got a single reading out of it - almost certainly not
                // plugged in. Switch to the drive encoders for the rest of this
                // run. seeded is still false, so the encoder path below will
                // establish its own baseline cleanly on the next tick.
                useTrackingWheel = false;
            }
        } else {
            // No tracking wheel, so fall back to the drive motors' own
            // encoders. This works, but powered wheels slip, so position
            // drifts more over the course of a match.
            //
            // Both sides get averaged together. During a turn one side goes
        ...
        // Split this tick's forward movement into how much of it was along X
        // and how much along Y, based on which way we were facing.
        //
        // This uses sin for X and cos for Y, which looks backwards if you're
        // used to math class. That's because we use the compass convention
        // (heading 0 = +Y, clockwise positive) rather than the math convention
        // (heading 0 = +X, counter-clockwise positive), so that heading can be
        // imu->get_rotation() directly with no conversion. Getting this
        // backwards would make the tracked position rotate the opposite way
        // from how turn_degrees() actually turns the robot.
        odomMutex.take();
