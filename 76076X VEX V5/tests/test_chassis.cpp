...
// The gear cartridge in a V5 motor decides how many encoder ticks it counts
// per revolution, and drive_distance() converts inches to ticks using that
// number. Get it wrong and every autonomous movement is off by a constant
// factor (3x between blue and green) with nothing to indicate why - the
// robot just drives the wrong distance. These are fixed hardware facts, so
// pin them down rather than trusting the lookup stays correct.
static void test_ticks_per_rev_matches_cartridge_color() {
    ...
    // And TICKS_PER_INCH should follow from it and the wheel size.
...
// Converts a distance in inches into the centidegree reading a tracking
// wheel of TRACKING_WHEEL_DIAMETER_INCH would report after rolling that far.
// Mirrors the conversion Chassis::set_tracking_wheel() derives internally.
...
// The same 10 inches of wheel travel, but with the robot facing +X instead
// of +Y. This is what actually pins down the sin/cos split in odomLoop():
// at heading 0 both the compass and standard-math conventions agree, so a
// forward-only test can't tell them apart. At heading 90 they disagree
// completely - compass convention puts the movement on +X, math convention
// would put it on +Y (or -X, depending on the sign error).
