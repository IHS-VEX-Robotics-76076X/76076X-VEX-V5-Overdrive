...
// including its `inline namespace v5` wrapper, so both pros::MotorGears and
// pros::v5::MotorGears resolve exactly as they do on device.
//
// The mock has no motor dynamics to simulate, so the cartridge is stored and
// readable but doesn't change how the fake motors behave. What it IS used for
// is config.hpp's TICKS_PER_REV lookup, and that conversion math is very much
